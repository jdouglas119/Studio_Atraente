...
begin  //Multiplicando o valor pela quantidade e informando que é pra baixar a quant. no estoque de acordo com a situação P
    ...
    try
       StrToDate(niver.Text);
          ...
          begin
             ShowMessage ('Data de Aniversário do Cliente está em branco - VERIFIQUE!');
             exit;
          end;
    end;
...
begin//Impressão da Comanda de Serviço
...
//Extraindo o mês da data de nascimento para gerar desconto no mês do aniversário
   ...
   if Mes = Hoje then
      begin
         ...
         frm_servico.label28.caption := 'Cliente Aniversariante no MÊS tem Desconto !';
      end
      else
...
// Alterando dados na tabela de itens
   if dt_studio.tb_movservicoSITUACAO.Value = 'FINALIZADO' then
      begin
         showmessage('SERVIÇO JÁ FINALIZADO USE OUTRA OPÇÃO!');
         exit;
      end
   else
   dt_studio.tb_comandas.edit;
...
begin
   if dt_studio.tb_movservicoSITUACAO.Value = 'ABERTO' then
      begin
         showmessage('SERVIÇO ESTÁ ABERTO USE OUTRA OPÇÃO!!');
         exit;
      end
   else
   if dt_studio.tb_movservicoSITUACAO.Value = 'FINALIZADO' then
...
begin
   ...
   If Application.MessageBox('Confirma Alteração de Serviços FINALIZADOS? ', 'Confirmação',
   Mb_IconQuestion + Mb_OkCancel + MB_DefButton1) = IdOk then
   else
   exit;
...
begin//Verificando se o Caixa já foi Aberto
   sbutton8.Enabled := false;
...
begin
   ...
   If Application.MessageBox('Confirma Alteração de Serviços ABERTOS? ', 'Confirmação',
   Mb_IconQuestion + Mb_OkCancel + MB_DefButton1) = IdOk then
   else
   exit;
...
// excluindo dados na tabela de itens
   ...
   if dt_studio.tb_comandasATENDENTE.AsString <> '' then
    begin
       Showmessage('EXISTEM ITENS QUE FAZEM REFERÊNCIA A ESSE SERVIÇO, EXCLUA PRIMEIRO OS ITENS!');
       id_serv.SetFocus;
    end
    else
...
// Grava registros na digitação na base de dados//
   ...
   if Mes = Hoje then
     ...
     if (dt_studio.tb_movservicoNOM_CLI.asstring = dt_studio.tb_cadcliNOM_CLI.asstring)
         ...
         begin
            frm_msg.showmodal;
            {MSG := 'ATENÇÃO!'+#13+'CLIENTE JÁ ESTÁ COM 10 OU MAIS VISITAS'+#13+'HAVERÁ UM DESCONTO NESSE SERVIÇO DE 5%!'
