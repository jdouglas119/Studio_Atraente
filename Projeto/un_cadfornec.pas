...
begin //Cancelando digitação das informações no banco
...
function ValidaCNPJ(CNPJ: string): Boolean; // Função para Validar CNPJ
var
...
begin //Validando CNPJ na digitação
...
begin //Alterando as informações no banco
...
begin
   ...
   Case Key of
        ...
        begin  //Cancelando informações no banco
           If Application.Messagebox('Confirma Cancelamento de Fornecedores? ',
           'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
           else
           exit;
   ...
   Case Key of
     ...
     begin     //Gravando informações no banco
           ...
           exit;
           if dt_studio.tb_cadfornecNOM_FORNEC.AsString = '' then //Informando os campos para preenchimento obrigatório
        begin
        ...
        if dt_studio.tb_cadfornecCNPJ_FORNEC.AsString = '' then //Informando os campos para preenchimento obrigatório
        begin
           Application.MessageBox('CAMPO CNPJ NÃO PODE SER VAZIO','AVISO',MB_ICONINFORMATION);
           cnpj.SetFocus;
           exit;
        end
        else
...
begin //Mudando a cor da fonte conforma a situação escolhida
