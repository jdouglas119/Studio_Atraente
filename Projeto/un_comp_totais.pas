...
begin//Gravando registros na tabela
   If Application.Messagebox('Confirma Gravação do Valor do Mês ? ', 'Confirmação',
...
begin//Cancelar registros na tabela
   If Application.Messagebox('Confirma Cancelamento do Valor do Mês ? ', 'Confirmação',
   mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
   dbgrid1.ReadOnly := true;
...
begin//Alterando registros na tabela
   If Application.Messagebox('Confirma Alteração do Valor do Mês ? ', 'Confirmação',
   mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
...
begin//Inserindo registros na tabela
   If Application.Messagebox('Confirma Lançamento do Valor do Mês ? ', 'Confirmação',
   mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
   dbgrid1.ReadOnly := false;
...
begin// Exortando movimentos do caixa
   dt_studio.tb_comp_totais.Edit;
   If Application.Messagebox('Confirma Exportação dos Valores? ', 'Confirmação',
