...
begin //Cancelando informações de digitação na tabela
   If Application.Messagebox('Confirma Cancelamento de Correção no Estoque? ',
...
begin // Inserindo registros na tabela
   If Application.Messagebox('Confirma Inclusão de Correção no Estoque? ',
   'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
...
begin //Gravando informações na tabela para mais
   If Application.Messagebox
   ('Confirma Gravação de Correção no Estoque para mais? ', 'Confirmação',
   ...
   if dt_studio.tb_cadtiposervTIPO.AsString = 'P' then //Gravando informações na tabela de produtos e serviços
      begin
...
begin //Gravando informações na tabela para menos
   If Application.Messagebox
   ('Confirma Gravação de Correção no Estoque para Menos? ', 'Confirmação',
   mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
   id_reg.ReadOnly := true;
...
begin //Mudando a cor da fonte de acordo com a escolha do usuário
...
begin //Informando se o produto está INATIVO
