...
begin //Cancelando digitação das informações na tabela
...
begin //Alterando informações na tabela
   If Application.Messagebox('Confirma Alteração da Forma de Pagamento? ',
   'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
   else
   exit;
...
begin
   Case Key of
        ...
        begin  //Inluindo informações na tabela
           If Application.Messagebox('Confirma Inclusão da Forma de Pagamento? ',
           'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
           else
           exit;
           id_formapag.ReadOnly := false;
   ...
   Case Key of
        ...
        begin  //Excluindo informações na tabela
           If dt_studio.tb_formapag.RecordCount <> 0 then
           If Application.Messagebox('Confirma Exclusão?', 'Confirmar',
           mb_Iconquestion + MB_YesNo + Mb_DefButton1) = IdYes Then
           begin
   ...
   Case Key of
        ...
        begin  //Cancelando informações na tabela
           If Application.Messagebox('Confirma Cancelamento da Forma de Pagamento? ',
           'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
           else
           exit;
   ...
   Case Key of
        ...
        begin  //Gravando informações na tabela
           If Application.Messagebox('Confirma Gravação da Forma de Pagamento? ',
           'Confirmação', mb_Iconquestion + Mb_OkCancel + Mb_DefButton1) = IdOk then
           else
           exit;
           if dt_studio.tb_formapagFORMA_PAG.AsString = '' then
           begin
              Application.MessageBox('CAMPO FORMA_PAG NÃO PODE SER VAZIO','AVISO',MB_ICONINFORMATION);
              nom_forma.SetFocus;
              exit;
           end
           else
...
begin //Gravando informações na tabela
   ...
   exit;
   if dt_studio.tb_formapagFORMA_PAG.AsString = '' then //Informando campo para preenchimento obrigatório
   begin
       Application.MessageBox('CAMPO FORMA_PAG NÃO PODE SER VAZIO','AVISO',MB_ICONINFORMATION);
       nom_forma.SetFocus;
       exit;
