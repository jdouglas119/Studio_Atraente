...
type
  Tfrm_rec_caixa = class(TForm)
    ...
    procedure pesqClick(Sender: TObject);
...
begin //Comandos para inserir uma cor no grid em uma determinada coluna//
    if (Column.Field.FieldName = 'COMANDA') then
...
begin //Definindo a situação de acordo com a baixa da comanda
 //Comandos para inserir uma cor no grid em uma determinada coluna//
...
// Gravar registros na base de dados//
  ...
  exit;
  dt_studio.tb_rec_caixaCLIENTE.AsString := edit4.Text;
...
begin//Abrindo as tabelas
   data_pesq.text := DateToSTr(Date);//data_mov.text := DateToSTr(Date);
...
procedure Tfrm_rec_caixa.RadioGroup1Click(Sender: TObject);
begin
   edit6.SetFocus;
end;
...
procedure Tfrm_rec_caixa.SpeedButton2Click(Sender: TObject);
var sql : string;
    vr_cred : currency;
    vr_deb : currency;
    vr_din : currency;
...
begin//Pesquisando por codigo do serviço e pelo nome do cliente
   dt_studio.tb_rec_caixa.Sql.Clear;
   dt_studio.tb_rec_caixa.Sql.Add('select * from REC_CAIXA');
   if (RadioGroup1.ItemIndex = 0) then
   Sql := 'where COMANDA like ' + QuotedStr('%' + Edit6.text + '%')
   else
   if (RadioGroup1.ItemIndex = 1) then
   Sql := 'where CLIENTE like ' + QuotedStr('%' + Edit6.text + '%');
   dt_studio.tb_rec_caixa.Sql.Add(Sql);
   dt_studio.tb_rec_caixa.Sql.Add('order by COMANDA');
   dt_studio.tb_rec_caixa.open;
   vr_cred := 0;
   vr_deb := 0;
   vr_din := 0;
...
procedure Tfrm_rec_caixa.pesqClick(Sender: TObject);
var sql : string;
